...
Sau mỗi lần export hoặc crop, app tạo thêm bundle `viewer` cho Windows 3D Viewer.
Texture trong bundle này được giới hạn cạnh dài tối đa 4096 px để dễ nạp hơn; raw OBJ,
MTL và texture gốc vẫn được giữ nguyên ở độ phân giải cao. Khi mở bằng 3D Viewer,
hãy chỉ mở file `.obj` trong thư mục `viewer` và giữ `.mtl` và `.jpg` cạnh file đó.
